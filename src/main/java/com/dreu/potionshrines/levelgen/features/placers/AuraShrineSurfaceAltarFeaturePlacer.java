...
public class AuraShrineSurfaceAltarFeaturePlacer extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        if (canPlaceBlock(context.level(), context.origin())
                ...
                && context.level().setBlock(context.origin().above(3), PSBlocks.AURA_SHRINE.get().defaultBlockState(), 11)) {

            context.level().setBlock(context.origin().above(2), PSBlocks.AURA_SHRINE_BASE.get().defaultBlockState().setValue(HALF, Half.TOP), 11);
            context.level().setBlock(context.origin().above(), PSBlocks.AURA_SHRINE_BASE.get().defaultBlockState().setValue(HALF, Half.BOTTOM), 11);
